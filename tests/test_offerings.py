"""domain/offerings.py holds prices, so its arithmetic and its validation are
...
def test_money_formatting(cents, expected):
    from domain.offerings import money
...
def test_title_is_required(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, OfferingError
    with pytest.raises(OfferingError):
        create(provider["id"], Draft(title="   "))


def test_duration_must_tile_a_grid(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, OfferingError, SLOT_MULTIPLES
    with pytest.raises(OfferingError):
        create(provider["id"], Draft(title="Odd", duration_min=37))
    for d in SLOT_MULTIPLES:
        assert create(provider["id"], Draft(title=f"Fine {d}", duration_min=d))


def test_price_cannot_be_negative(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, OfferingError
    with pytest.raises(OfferingError):
        create(provider["id"], Draft(title="Owed money", price_cents=-1))


def test_zero_price_is_allowed(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, get
    oid = create(provider["id"], Draft(title="Free intro", price_cents=0))
...
def test_list_hides_deactivated_by_default(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, deactivate, list_for_provider
    oid = create(provider["id"], Draft(title="Temporary"))
...
def test_deactivate_never_deletes(ctx, provider):
    ...
    from domain.offerings import create, OfferingDraft as Draft, deactivate, get
    oid = create(provider["id"], Draft(title="Retired"))
    deactivate(oid, provider["id"])
...
def test_cannot_touch_someone_elses_offering(ctx, provider, client):
    from domain.offerings import create, OfferingDraft as Draft, update, deactivate, OfferingError
    from tests.conftest import register
    _, other = register(client, "other@test.local", role="provider")
    oid = create(provider["id"], Draft(title="Mine"))
    with pytest.raises(OfferingError):
...
def test_update_validates_the_merged_row(ctx, provider):
    ...
    from domain.offerings import create, OfferingDraft as Draft, update, OfferingError
    oid = create(provider["id"], Draft(title="Session", duration_min=30))
    with pytest.raises(OfferingError):
...
def test_update_with_nothing_to_change_is_a_no_op(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, update
    oid = create(provider["id"], Draft(title="Unchanged"))
    assert update(oid, provider["id"])["title"] == "Unchanged"


def test_grouping_follows_the_category_order(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, grouped_for_provider, CATEGORIES
    create(provider["id"], Draft(title="W", category="Web development"))
    create(provider["id"], Draft(title="S", category="Software engineering"))
    create(provider["id"], Draft(title="C", category="Computer science"))
...
def test_unknown_category_still_appears(ctx, provider):
    ...
    from domain.offerings import create, OfferingDraft as Draft, grouped_for_provider
    create(provider["id"], Draft(title="Odd one", category="Underwater basketry"))
...
def test_public_view_formats_money_and_flags_free(ctx, provider):
    from domain.offerings import create, OfferingDraft as Draft, get, public_view
    paid = public_view(get(create(provider["id"], Draft(title="Paid", price_cents=4500))))
    free = public_view(get(create(provider["id"], Draft(title="Free", price_cents=0))))
